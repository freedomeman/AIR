...
// {
//     // 1. 临时注释无限等待（先排除阻塞问题）
//     // TickType_t xTimeout = pdMS_TO_TICKS(10);
//     // if (ulTaskNotifyTake(pdTRUE, xTimeout) != pdPASS) return;

//     // 2. 临时注释所有数据处理逻辑（只保留空任务）
//     vTaskDelay(pdMS_TO_TICKS(1)); // 让任务主动让出CPU

//     // 【关键测试】：仅保留空任务+延时，开启IMU任务，看电机是否还报0x02错误
//     // - 若仍报错：问题在IMU的中断（SPI/DMA/IMU芯片中断）与电机冲突
//     // - 若不报错：问题在IMU的数据处理逻辑（临界区/运算/共享数据）
// }
