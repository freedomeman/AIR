...
//射频等级 拨弹电机
fp32 shoot_grigger_grade[6] = {0, 5.0f * grigger_speed_to_radio, 13.75f * grigger_speed_to_radio, 15.0f * grigger_speed_to_radio, 28.0f * grigger_speed_to_radio, 40.0f * grigger_speed_to_radio};

//拨盘等级 摩擦轮等级12
uint8_t grigger_speed_grade;
uint8_t fric_speed_grade;

uint16_t snail_pwm[3];
...
//uint16_t pwm_my=1000;
void Shoot::set_mode()
{
    static int8_t last_s = RC_SW_UP; //记录上一次遥控器按键值

    //shoot_fric1_on(pwm_my);
    //上拨判断， 一次开启，再次关闭
    ...
    // }
    //保证SNAIL电机到达转速
    if (shoot_mode == SHOOT_READY_FRIC && snail_pwm[0] >= 1300)
    {
        fric_status = TRUE;
        shoot_mode = SHOOT_READY_BULLET;
    }
...
// #endif
    ...
    // else
    {
        ...
        temp_colling_detal = id1_17mm_cooling_limit - id1_17mm_cooling_heat;

        //根据当前热量和射速修改等级,确保不会因超限扣血,
        //热量 当剩余热量低于30,强制制动
        if (id1_17mm_cooling_limit - id1_17mm_cooling_heat <= 30)
        {
            if (grigger_speed_grade != 0)
            {
...
void Shoot::solve()
{
    ...
    if (shoot_mode == SHOOT_STOP)
    {
        ...
        if (snail_pwm[0] >1000){snail_pwm[0] --;}
        if (snail_pwm[1] >1000){snail_pwm[1] --;}
        if (snail_pwm[2] >1000){snail_pwm[2] --;}
...
#endif
    //发送电流
    shoot_fric1_on(snail_pwm[0]);
    shoot_fric2_on(snail_pwm[1]);
    shoot_fric3_on(snail_pwm[2]);
